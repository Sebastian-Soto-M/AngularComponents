...
import { DATE_FORMAT } from '../util/input.constants';
import { IUserProfile } from '../entities/user-profile.model';
...
export class UserProfileService {
  ...
  findByUser(id: number): Observable<EntityResponseType> {
    return this.http
      .get<IUserProfile>(`${this.resourceUrl}/user/${id}`, {
        observe: 'response',
      })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<IUserProfile[]>(this.resourceUrl, {
        params: options,
        observe: 'response',
      })
      .pipe(
  ...
  protected convertDateFromClient(userProfile: IUserProfile): IUserProfile {
    const copy: IUserProfile = Object.assign({}, userProfile, {
      birthDate:
        userProfile.birthDate && userProfile.birthDate.isValid()
          ? userProfile.birthDate.format(DATE_FORMAT)
          : undefined,
    });
    return copy;
  }
